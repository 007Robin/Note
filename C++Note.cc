...
虚拟存储器是个逻辑模型，它把user给的一个虚地址，由辅助硬件将它映射到主存中到某个单元，该主存单元地址叫做物理地址，实地址。
块表TLB在cache里，慢表Page在主存里, 快表是慢表的副本。
收到虚拟地址后，先访问主存，查询页表，段表，进行虚实地址转换. 用了TLB后则，
先查TLB，命中则去Cache处理数据, 未命中才去Page里找，且该步需要页面调度，这期间也正好更新了TLB和Page。

cache命中次数 与 地址映射关系和替换算法都有关。

3级缓存
L1：64kb, 1~3 clock
L2: 256kb, 15 clock
L3: 8~30Mb, 50-100 clock
